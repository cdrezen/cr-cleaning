...
class Player implements Serializable, Writable {
	...
	public boolean isAnyRelevantEmptyOrNull() {
		return StringUtils.isEmpty(utag) || StringUtils.isEmpty(ctag) || StringUtils.isEmpty(deck); // ||
																									// StringUtils.isEmpty(evo)
																									// ||
																									// StringUtils.isEmpty(tower);
	}
...
class Battle implements Serializable, Writable {
	...
	public boolean isAnyEmptyOrNull() {
		...
		// if(f.getType().isAssignableFrom(String.class) && f.get(this) == "") return
		// true;
		...
		if (StringUtils.isEmpty(date) || StringUtils.isEmpty(game) || StringUtils.isEmpty(mode)
				|| StringUtils.isEmpty(type) || players == null || players.isEmpty())
			return true;

		for (Player player : players) {
			if (player.isAnyRelevantEmptyOrNull())
				return true;
		}
